...
```tsx
function undirtyFields(fieldBag: FieldBag): void;
```
...
const {
  ...
  onSubmit: async values => {
    ...
    if (someCondition) {
      undirtyForm(fields); // set form state to 'clean'
    }
    return submitSuccess();
  },
  undirtyAfterSubmit: false, // just for illustrative purposes, the default is false
});
```
